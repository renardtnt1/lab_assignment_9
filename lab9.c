#include <stdio.h>

// RecordType
struct RecordType
{
	int		id;
	char	name;
	int		order; 
};

// Fill out this structure
struct HashType
{
	struct RecordType *data; // Data stored in the node
    struct HashType *next;   // Pointer to the next node
};

// Compute the hash function
int hash(int x, int hashSz)
{
    // Ensure hashSz is positive to avoid division by zero
    if (hashSz <= 0) {
        // Handle invalid hash size
        return -1; // Return -1 indicating an error
    }
    
    // Simple hash function
    return x % hashSz;
}

// parses input file to an integer array
int parseData(char* inputFileName, struct RecordType** ppData)
{
	FILE* inFile = fopen(inputFileName, "r");
	int dataSz = 0;
	int i, n;
	char c;
	struct RecordType *pRecord;
	*ppData = NULL;

	if (inFile)
	{
		fscanf(inFile, "%d\n", &dataSz);
		*ppData = (struct RecordType*) malloc(sizeof(struct RecordType) * dataSz);
		// Implement parse data block
		if (*ppData == NULL)
		{
			printf("Cannot allocate memory\n");
			exit(-1);
		}
		for (i = 0; i < dataSz; ++i)
		{
			pRecord = *ppData + i;
			fscanf(inFile, "%d ", &n);
			pRecord->id = n;
			fscanf(inFile, "%c ", &c);
			pRecord->name = c;
			fscanf(inFile, "%d ", &n);
			pRecord->order = n;
		}

		fclose(inFile);
	}

	return dataSz;
}

// prints the records
void printRecords(struct RecordType pData[], int dataSz)
{
	int i;
	printf("\nRecords:\n");
	for (i = 0; i < dataSz; ++i)
	{
		printf("\t%d %c %d\n", pData[i].id, pData[i].name, pData[i].order);
	}
	printf("\n\n");
}

// display records in the hash structure
// skip the indices which are free
// the output will be in the format:
// index x -> id, name, order -> id, name, order ....
void displayRecordsInHash(struct HashType *pHashArray, int hashSz)
{
	int i;

    for (i=0; i<hashSz; ++i)
    {
        printf("Index %d -> ", i);
        struct HashType *current = &pHashArray[i];
        while (current != NULL) {
            printf("%d %c %d -> ", current->data->id, current->data->name, current->data->order);
            current = current->next;
        }
        printf("\n");
    }
}

int main(void)
{
	struct RecordType *pRecords;
    int recordSz = 0;

    // Assuming hash table size
    int hashSz = 10;

    // Initialize hash table
    struct HashType *pHashArray = (struct HashType*)malloc(sizeof(struct HashType) * hashSz);
    for (int i = 0; i < hashSz; i++) {
        pHashArray[i].next = NULL;
    }

    recordSz = parseData("input.txt", &pRecords);
    printRecords(pRecords, recordSz);

	//Your Implementation
    // Hash records
    for (int i = 0; i < recordSz; ++i)
    {
        int hashedIndex = hash(pRecords[i].id, hashSz);
        struct HashType *newNode = (struct HashType*)malloc(sizeof(struct HashType));
        newNode->data = &pRecords[i];
        newNode->next = pHashArray[hashedIndex].next;
        pHashArray[hashedIndex].next = newNode;
    }

    // Display records in hash table
    displayRecordsInHash(pHashArray, hashSz);

    // Free allocated memory
    // Assuming you'll need to free the memory used for records as well
    for (int i = 0; i < recordSz; i++) {
        free(&pRecords[i]);
    }
    free(pRecords);
    free(pHashArray);
    
    return 0;
}